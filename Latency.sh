#!/bin/bash
#Parameters
impl="MAPTPERF-LAT-Jool-BME-10G-1000_CEs-EAbits13-Fg100-frameSize84-16CoresAtDUT" # name of the tested implementation (used for logging)
dir="b" # valid values: b,f,r; b: bidirectional, f: forward (Left to Right, 6 --> 4), r: reverse (Right to Left, 6 <-- 4) 
r=891979 # frame rate determined by throughput measurement
...
do
	...
	echo "Running experiment #$N..." >> latencytest.log
	echo "Command line is: ./build/maptperf-lat $fs $r $xpts $to $n $m $delay $tags"
	echo "Command line is: ./build/maptperf-lat $fs $r $xpts $to $n $m $delay $tags" >> latencytest.log
	# Execute the test program
	./build/maptperf-lat $fs $r $xpts $to $n $m $delay $tags > temp.out 2>&1
