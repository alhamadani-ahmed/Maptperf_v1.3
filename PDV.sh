#!/bin/bash
#Parameters
impl="MAPTPERF-PDV-Jool-BME-10G-1000_CEs-EAbits13-Fg100-frameSize84-16CoresAtDUT" # name of the tested implementation (used for logging)
dir="b" # valid values: b,f,r; b: bidirectional, f: forward (Left to Right, 6 --> 4), r: reverse (Right to Left, 6 <-- 4) 
r=8
#r=891979 # frame pdv determined by thropughput measurement
fs=84 # IPv6 frame size; IPv4 frame size is always 20 bytes less 
xpts=6 # duration (in seconds) of an experiment instance
to=2000 # timeout in milliseconds
n=10 # foreground traffic, if ( frame_counter % n < m ) 
m=5 # E.g. n=m=2 is all foreground traffic; n=2,m=0 is all background traffic; n=10,m=9 is 90% fg and 10% bg
...
do
	...
	echo "Command line is: ./build/maptperf-pdv $fs $r $xpts $to $n $m 0"
	echo "Command line is: ./build/maptperf-pdv $fs $r $xpts $to $n $m 0" >> pdvtest.log
	# Execute the test program
	./build/maptperf-pdv $fs $r $xpts $to $n $m 0 > temp.out 2>&1
